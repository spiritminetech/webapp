...
const assignExistingTasksToEmployee2 = async () => {
  try {
    console.log('📝 Assigning existing tasks to Employee ID 2...\n');
    ...
    projects.forEach(p => {
      console.log(`   - ${p.projectName || 'Unnamed'} (ID: ${p.id})`);
    });
    ...
    // Use the first project with a valid ID
    const project = projects.find(p => p.id !== undefined && p.id !== null);
    if (!project) {
      console.log('❌ No project with valid ID found');
      return;
    }

    console.log(`\n✅ Using project: ${project.projectName || 'Unnamed'} (ID: ${project.id})`);

    // Find existing tasks for this project
    const tasks = await Task.find({ projectId: project.id }).limit(5);
    console.log(`\n📋 Found ${tasks.length} tasks for this project:`);
    tasks.forEach(t => {
    ...
    for (let i = 0; i < tasksToAssign.length; i++) {
      ...
      const assignment = new WorkerTaskAssignment({
        ...
        sequence: i + 1,
        progressPercent: 0,
        workArea: task.description || 'General Work Area',
        estimatedHours: 8,
    ...
    console.log(`   Project: ${project.projectName || 'Unnamed'} (ID: ${project.id})`);
...
const main = async () => {
  await connectDB();
  await assignExistingTasksToEmployee2();
